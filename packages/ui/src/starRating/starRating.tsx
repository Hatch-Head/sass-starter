...
export const Star = ({ active, className = "" }: StarProps) => {
  return (
    <svg
      width="18"
      height="18"
...
const StarRating = ({ value = 0, onChange, total = 5 }: StarRatingProps) => {
  ...
  return (
    <div className="star-rating flex space-x-1" onMouseUp={handleMouseUp}>
      {[...Array(total)].map((_, index) => (
        <button
          type="button"
          key={index}
          className={index < roundValue ? "on" : "off"}
          onMouseDown={() => handleMouseDown(index)}
          onMouseOver={() => handleMouseOver(index)}
        >
          <Star active={index < roundValue} />
        </button>
      ))}
    </div>
  );
